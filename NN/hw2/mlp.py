...
def featurize(sentence: str, embeddings: gensim.models.keyedvectors.KeyedVectors) -> Union[None, torch.FloatTensor]:
    ...
    if len(vectors) == 0:
        return None
    # Print what vectors looks like
    print(f"vectors (type: {type(vectors)}, length: {len(vectors)})")
    print(f"  First element type: {type(vectors[0])}, shape: {vectors[0].shape if hasattr(vectors[0], 'shape') else 'N/A'}")
    print(f"  Sample first vector (first 5 values): {vectors[0][:5] if len(vectors[0]) > 5 else vectors[0]}")
    
    # Convert list of numpy arrays to numpy array, then to torch tensor, then compute the mean
    vectors_tensor = torch.from_numpy(np.stack(vectors).astype(np.float32))
    print(f"vectors_tensor after stacking (type: {type(vectors_tensor)}, shape: {vectors_tensor.shape})")
    
    result = torch.mean(vectors_tensor, dim=0)
    print(f"Final output (type: {type(result)}, shape: {result.shape})")
    print(f"  Sample first 5 values: {result[:5]}")
...
class SentimentClassifier(nn.Module):
    def __init__(self, embed_dim: int, num_classes: int, hidden_dims: List[int], activation: str = 'Sigmoid'):
        ...
        if len(hidden_dims) == 0:
            # No hidden layers, just one layer from embed_dim to num_classes
            self.linears.append(nn.Linear(embed_dim, num_classes))
        else:
            # First layer: embed_dim to hidden_dims[0]
            self.linears.append(nn.Linear(embed_dim, hidden_dims[0]))
            
            # Hidden layers: hidden_dims[i] to hidden_dims[i+1]
            for i in range(len(hidden_dims) - 1):
                self.linears.append(nn.Linear(hidden_dims[i], hidden_dims[i + 1]))
            
            # Output layer: hidden_dims[-1] to num_classes
...
def accuracy(logits: torch.FloatTensor, labels: torch.LongTensor) -> torch.FloatTensor:
    ...
    # Get predictions by taking argmax along the num_classes dimension
    predictions = torch.argmax(logits, dim=1)
    # Compare predictions with labels and convert to 0s and 1s
